...
create type family_status_enum as enum('Single','Married','Divorced','Widow','Separated','Other');
create type hous_type_enum as enum('House','Apartment');
...
select first_name,house_type from owners inner join houses h on owners.owner_id = h.owner_id where length(first_name)=(select max(length(first_name)) from owners);
-- 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select exists(select 1 from houses where price between 1500 and 2000) as result;
-- 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select address_id,houses from houses where address_id in (5,6,7,8,9,10);
-- 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select house_type, o.first_name as owner_name,c.first_name as customer_name,a.name as agency_name from houses join owners o on o.owner_id = houses.owner_id join rent_info ri on o.owner_id = ri.owner_id join customers c on o.first_name = c.first_name join agencies a on houses.address_id = a.address_id;
-- 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
select customers.first_name,customers.date_of_birth from customers where date_of_birth>'1990-01-01' order by customer_id limit 15 offset 10;
-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select house_type,houses.rating,o.first_name as owner_name from houses join owners o on o.owner_id = houses.owner_id order by houses.rating asc ;
...
select name,agencies.address_id,h.address_id from agencies inner join houses h on agencies.address_id = h.address_id join rent_info ri on agencies.agency_id = ri.agency_id where name='My House';
-- 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select houses,first_name,address_id from houses join owners o on o.owner_id = houses.owner_id where furniture='True';
-- 10.Кленти жок уйлордун баарын жана анын адрессин жана ал уйлор кайсыл агентсвога тийешелуу экенин чыгар.  ===================
select house_type,price, rating, description, room,
       city, region, street from houses h
                                     inner join agencies a on a.agency_id=house_id
                                     inner join addresses a1 on a1.address_id=house_id
                                     left join customers c on c.customer_id=house_id
...
select * from houses order by rating limit 1 offset (select count(*) from houses)/2;
-- 13.Уйлору жок киленттерди, клиенттери жок уйлорду чыгар. ==========
select * from customers where customer_id not in (select distinct customer_id from rent_info);
select * from houses where house_id not in (select distinct house_id from rent_info);
...
select o.first_name,c.first_name from owners o join houses h on o.owner_id = h.owner_id
                                               join rent_info ri on h.house_id = ri.house_id
...
select first_name, nationality,marital_status from customers where nationality='Kyrgyz' and marital_status='Married' ;
-- 18.Эн коп болмолуу уйду жана анын адресин ал уй кайсыл ownerге таандык ошону чыгарыныз.
select room,address_id,o.first_name from houses h join owners o on o.owner_id = h.owner_id order by h.room desc limit 1;
-- 19.Бишкекте жайгашкан уйлорду жана клиентерин кошо чыгарыныз.
select a.city,c.first_name from houses h join rent_info ri on h.house_id = ri.house_id
                                         join customers c on c.customer_id = ri.customer_id
...
select house_id,(check_out-rent_info.check_in) as rental_duration from rent_info order by rental_duration desc  limit 1;
-- 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select o.first_name,houses from houses
                                    join owners o on o.owner_id = houses.owner_id
order by price desc limit 1;
-- 23.Бир региондо жайгашкан баардык агентстволорду чыгарыныз
select a.agency_id,a.name as agency_name,
       a.phone_number,ad.city,ad.region,ad.street from agencies a
                                                           join addresses ad on ad.address_id = a.address_id
where ad.region= 'Specific Region';
...
select o.first_name as owner_first_name,
       ...
       h.room as house_rooms,
       h.furniture as house_furniture,
       a.city as house_city,
       a.region as house_region,
       a.street as house_street
from owners o join houses h on o.owner_id = h.owner_id
              join addresses a on h.address_id = a.address_id
where DATE_PART('year', age(o.date_of_birth)) between 40 and 60;